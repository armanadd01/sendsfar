...
import { fetchRequestedTransfers, formatFileSize } from '@/lib/api';
...
type RequestedTransfersProps = {
  searchQuery: string;
};

export default function RequestedTransfers({ searchQuery }: RequestedTransfersProps) {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const { data: transfers, total } = await fetchRequestedTransfers({ 
        page: currentPage, 
        limit: itemsPerPage, 
        search: searchQuery 
      });
      ...
      console.error('Error fetching requested transfers:', error);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="">
      {isLoading ? (
        ...
        <div className="text-center py-8">No requested transfers found</div>
      )}
    </div>
  );
}
